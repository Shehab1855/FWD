...
def get_filters():
    ...
    while True:
        month =input("For which month ? (all, january, february, march, april, may , june)? ").lower()
    ...
    while True:
        day = input("For which day ? (all, monday, tuesday, wednesday, thursday, friday, saturday, sunday) ").lower()
...
def raw_data_request(df):
    x,y=0,5
    while True:
            raw_data_request = input("Would you like to see some raw data? (yes,no)").lower()
...
def main():
    while True:
        ...
        user_stats(df)
        raw_data_request(df)  #display raw data  
